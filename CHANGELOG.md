# Changelog

All notable changes to the "tiddlyedit" extension will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).


## [0.2.8] - 2025-12-11

### Added

- Option to send cursor position offset to browser when opening and saving tiddlers (disabled by default), which allows the browser to highlight the cursor row when editing tiddlers in VS Code.


## [0.2.7] - 2025-12-11

### Added

- Enable editing system tiddlers and tiddlers with special characters in title (contributed by @LovelyShaun)

## [0.2.6] - 2025-10-07

### Added
- Enable auto save when editing tiddler in Visual Studio Code (disabled by default)

## [0.2.5] - 2025-09-24

### Added
- Add and delete tiddler tags
- Open tiddler links in VS Code using Ctrl+Click or Cmd+Click on a tiddler link

## [0.1.0] - 2025-08-14

### Added
- Open a tiddler in browser and edit a tiddler in VS Code from browser
- Auto completion using [Auto Complete](https://github.com/EvidentlyCube/TW5-AutoComplete) plugin for TiddlyWiki
- Added preview support for Tiddlers

### Improved
- Improved meta information display for tiddlers
- Improved syntax highlighting for macros and widgets
